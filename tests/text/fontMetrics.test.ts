...
function parseGlyphCharsFromXml(xml: string): Record<number, { yOffset: number; height: number }> {
  const chars: Record<number, { yOffset: number; height: number }> = {};
  const pattern = /<char id="(\d+)"[^>]*height="(\d+)"[^>]*yoffset="(\d+)"/g;
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(xml)) !== null) {
    const id = Number(match[1]);
    const height = Number(match[2]);
    const yOffset = Number(match[3]);
    chars[id] = { yOffset, height };
  }
  return chars;
}
...
describe("fontMetrics isolation", () => {
  test("jf-dot-mplus12 XML exposes stable metrics for fallback inference", () => {
    const xml = readFileSync(join(ROOT, "examples/assets/fonts/jf-dot-mplus12/font.xml"), "utf8");
    expect(xml).toMatch(/<common[^>]*lineHeight="14"/);
    expect(xml).toMatch(/<common[^>]*base="11"/);

    const chars = parseGlyphCharsFromXml(xml);
    const inferred = inferBitmapFontBaseFromChars(chars, 14);
    expect(inferred).toBe(13);
  });
});
