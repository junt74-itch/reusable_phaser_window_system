...
describe("PhaserWindowInput wheel and drag contract", () => {
  test("registers and unregisters Phaser wheel and pointer listeners", () => {
    const source = readFileSync(join(ROOT, "src/input/PhaserWindowInput.ts"), "utf8");
    expect(source.includes('this.scene.input.on("wheel", this.wheelHandler)')).toBe(true);
    expect(source.includes('this.scene.input.off("wheel", this.wheelHandler)')).toBe(true);
    expect(source.includes('this.scene.input.on("pointerupoutside", this.pointerUpHandler)')).toBe(
      true,
    );
    expect(source.includes('this.scene.input.off("pointerupoutside", this.pointerUpHandler)')).toBe(
      true,
    );
    expect(source.includes("emitDragFromPhaser")).toBe(true);
    expect(source.includes("Math.trunc(totalDeltaY)")).toBe(true);
    expect(source.includes("this.emitPointerFromPhaser(pointer, \"repeated\", pointer.isDown)")).toBe(
      true,
    );
  });
});
