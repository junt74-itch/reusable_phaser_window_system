...
describe("bindWindowA11y", () => {
  ...
  test("emits selectionChanged with item fields for a captioner", () => {
    const listeners: Array<(index: number, item: { id: string; label: string } | null) => void> = [];
...
describe("a11y isolation", () => {
  test("WindowBase has no a11y, DOM, or EventEmitter API", () => {
    const windowBase = readFileSync(join(ROOT, "src/core/WindowBase.ts"), "utf8");
    expect(windowBase.includes("subscribeTransition")).toBe(true);
    expect(windowBase.includes("a11y")).toBe(false);
    expect(windowBase.includes("bindWindowA11y")).toBe(false);
    expect(windowBase.includes("EventEmitter")).toBe(false);
    expect(windowBase.includes("createElement")).toBe(false);
