...
export class WindowBase {
  ...
  public worldToContentLocal(worldX: number, worldY: number): { x: number; y: number } {
    const point = this.content.getLocalPoint(worldX, worldY);
  ...
  public contentLocalToWorld(localX: number, localY: number): { x: number; y: number } {
    ...
    const out = new Phaser.Math.Vector2(localX, localY);
  ...
  public open(durationMs?: number): Promise<void> {
    this.assertAlive();
    return this.transition.open(durationMs);
  }
  ...
  private relayout(): void {
    ...
    this.content.setPosition(this.contentBounds.x, this.contentBounds.y);
    this.renderer.resize(this.width, this.height);
    this.clipper.updateBounds(this.contentBounds);
  ...
  private applyOpennessPresentation(): void {
    ...
    this.root.setScale(1, scaleY);
    this.root.setPosition(Math.trunc(this.rootX), Math.trunc(this.rootY + offsetY));
    this.root.setVisible(this.visible && scaleY > 0);
