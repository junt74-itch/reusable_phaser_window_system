...
Bind shutdown with `bindFocusControllerToScene(scene, controller)` so restart/shutdown calls `dispose()` (release all). The Scene destroys its dimmer on shutdown.
